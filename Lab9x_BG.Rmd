...
```{r}
y <- sample.int(35,10, replace = TRUE)
...
f(2)
```
Obviously, we could have just cut-and-pasted the `cat("Hello, world!\n")` code three times to achieve the  same effect, but then we wouldn’t be programming, would we? Also, it would be un-neighb
...
+ has one *formal argument* named `num` with a *default value* of 2. The *formal arguments* are the arguments included in the function definition. The `formals()` function returns a list of all the formal arguments of a function
